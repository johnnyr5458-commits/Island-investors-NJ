...
export default function TopBar({ title, subtitle }: TopBarProps) {
  ...
  return (
    ...
    >
      <div>
        <h1 className="font-display text-xl font-semibold text-white leading-tight">{title}</h1>
        {subtitle && <p className="font-sans text-xs text-silver-600 mt-0.5">{subtitle}</p>}
      </div>
      <div className="flex items-center gap-3">
        <div
          className="hidden sm:flex items-center gap-1.5 font-sans text-[10px] text-silver-700 px-3 py-1.5 rounded-sm"
